<?php 
			...
			/**
			 * change_password_link is a database object that can persist, delete and create forms for the change_password_link table
			 *
			 */
			
			class change_password_link extends baseModel
			{
			    ...
			    */
				public function __construct($rewriting=false)
				{
					...
					$this->tableName="change_password_link";
					
					parent::__construct($rewriting);
					
					
				$this->arrayFields["idchange_password_link"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"PRI",
					...
					"formLabel"=>"idchange_password_link"
				);
			
				$this->arrayFields["cp_id"]=array
				(
					"type"=>"varchar(45)",
					"canBeNull"=>"YES",
					"key"=>"",
				...
				$this->arrayFields["cp_expiration"]=array
				(
					"type"=>"bigint(15)",
					"canBeNull"=>"YES",
					"key"=>"",
			    ...
			    */
				public function initFromDatas($arrayFilters)
				{
					...
					$sqlReq="SELECT * FROM change_password_link ".$theWhere." LIMIT 1";
			    ...
			    */
				public function delete()
				{
					if($this->isSavedElement)
					{
						...
						$sqlDelete="DELETE FROM change_password_link ".$theWhere."";
			    ...
			    */
				public function save($arrayFieldNames=null)
				{
					...
					$sqlInsert="INSERT INTO change_password_link(".implode(",",$toSave).") values(".implode(",",$toSaveBinding).")";
			    ...
			    */
				public function update($arrayFieldNames=null)
				{
					...
					if($this->isSavedElement)
					{
						...
						$sqlUpdate="UPDATE change_password_link SET ".implode(",",$toSave)." WHERE ".$this->getPrimaryKeyField()."=:".$this->getPrimaryKeyField()."";
			...
			/**
			 * change_password_linkDatasManager is a database object that list datas from the change_password_link table
			 *
			 */
			
			class change_password_linkDatasManager extends baseModel
			{
			    ...
			    */
				...
				public function refreshNumElements()
				{
					...
					$sqlCount="SELECT COUNT(*) AS numElements FROM change_password_link ".$this->getWhereClause();
				...
				{
					$sqlReq="SELECT * FROM change_password_link ".$this->getWhereClause()." ".$this->getLimitClause($page)."";
				...
				{
					$sqlReq="SELECT ".$columns." FROM change_password_link ".$supReq;
					$prepa=$this->dbh->prepare($sqlReq);
					if($qh)
					{$prepa->execute($qh);}
					else 
