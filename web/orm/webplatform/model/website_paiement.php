<?php 
			...
			/**
			 * website_paiement is a database object that can persist, delete and create forms for the website_paiement table
			 *
			 */
			
			class website_paiement extends baseModel
			{
			    ...
			    */
				public function __construct($rewriting=false)
				{
					...
					$this->tableName="website_paiement";
					
					parent::__construct($rewriting);
					
					
				$this->arrayFields["idwebsite_paiement"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"PRI",
					...
					"formLabel"=>"idwebsite_paiement"
				);
			
				$this->arrayFields["website_idwebsite"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"MUL",
					"default"=>"",
					"extra"=>"",
					"formLabel"=>"website_idwebsite"
				);
			
				$this->arrayFields["paiements_idpaiements"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"MUL",
					"default"=>"",
					"extra"=>"",
					"formLabel"=>"paiements_idpaiements"
				);
			
					
				$this->arrayRelations["paiements_idpaiements"]=array
				(
					"assoc_table"=>"paiements",
					"assoc_col"=>"idpaiements"
				);
			
				$this->arrayRelations["website_idwebsite"]=array
				(
					"assoc_table"=>"website",
					"assoc_col"=>"idwebsite"
				);
			    ...
			    */
				public function initFromDatas($arrayFilters)
				{
					...
					$sqlReq="SELECT * FROM website_paiement ".$theWhere." LIMIT 1";
			    ...
			    */
				public function delete()
				{
					if($this->isSavedElement)
					{
						...
						$sqlDelete="DELETE FROM website_paiement ".$theWhere."";
			    ...
			    */
				public function save($arrayFieldNames=null)
				{
					...
					$sqlInsert="INSERT INTO website_paiement(".implode(",",$toSave).") values(".implode(",",$toSaveBinding).")";
			    ...
			    */
				public function update($arrayFieldNames=null)
				{
					...
					if($this->isSavedElement)
					{
						...
						$sqlUpdate="UPDATE website_paiement SET ".implode(",",$toSave)." WHERE ".$this->getPrimaryKeyField()."=:".$this->getPrimaryKeyField()."";
			...
			/**
			 * website_paiementDatasManager is a database object that list datas from the website_paiement table
			 *
			 */
			
			class website_paiementDatasManager extends baseModel
			{
			    ...
			    */
				...
				public function refreshNumElements()
				{
					...
					$sqlCount="SELECT COUNT(*) AS numElements FROM website_paiement ".$this->getWhereClause();
				...
				{
					$sqlReq="SELECT * FROM website_paiement ".$this->getWhereClause()." ".$this->getLimitClause($page)."";
				...
				{
					$sqlReq="SELECT ".$columns." FROM website_paiement ".$supReq;
					$prepa=$this->dbh->prepare($sqlReq);
					if($qh)
					{$prepa->execute($qh);}
					else 
