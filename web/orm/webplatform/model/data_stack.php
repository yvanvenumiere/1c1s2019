<?php 
			...
			/**
			 * data_stack is a database object that can persist, delete and create forms for the data_stack table
			 *
			 */
			
			class data_stack extends baseModel
			{
			    ...
			    */
				public function __construct($rewriting=false)
				{
					...
					$this->tableName="data_stack";
					
					parent::__construct($rewriting);
					
					
				$this->arrayFields["iddata_stack"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"PRI",
					...
					"formLabel"=>"iddata_stack"
				);
			
				$this->arrayFields["ds_name"]=array
				(
					"type"=>"varchar(75)",
					"canBeNull"=>"YES",
					"key"=>"",
				...
				$this->arrayFields["ds_serialised_content"]=array
				(
					"type"=>"text",
					"canBeNull"=>"YES",
					"key"=>"",
					"default"=>"",
					"extra"=>"",
					"formLabel"=>"ds_serialised_content"
				);
			    ...
			    */
				public function initFromDatas($arrayFilters)
				{
					...
					$sqlReq="SELECT * FROM data_stack ".$theWhere." LIMIT 1";
			    ...
			    */
				public function delete()
				{
					if($this->isSavedElement)
					{
						...
						$sqlDelete="DELETE FROM data_stack ".$theWhere."";
			    ...
			    */
				public function save($arrayFieldNames=null)
				{
					...
					$sqlInsert="INSERT INTO data_stack(".implode(",",$toSave).") values(".implode(",",$toSaveBinding).")";
			    ...
			    */
				public function update($arrayFieldNames=null)
				{
					...
					if($this->isSavedElement)
					{
						...
						$sqlUpdate="UPDATE data_stack SET ".implode(",",$toSave)." WHERE ".$this->getPrimaryKeyField()."=:".$this->getPrimaryKeyField()."";
			...
			/**
			 * data_stackDatasManager is a database object that list datas from the data_stack table
			 *
			 */
			
			class data_stackDatasManager extends baseModel
			{
			    ...
			    */
				...
				public function refreshNumElements()
				{
					...
					$sqlCount="SELECT COUNT(*) AS numElements FROM data_stack ".$this->getWhereClause();
				...
				{
					$sqlReq="SELECT * FROM data_stack ".$this->getWhereClause()." ".$this->getLimitClause($page)."";
				...
				{
					$sqlReq="SELECT ".$columns." FROM data_stack ".$supReq;
					$prepa=$this->dbh->prepare($sqlReq);
					if($qh)
					{$prepa->execute($qh);}
					else 
