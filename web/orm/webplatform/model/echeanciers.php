<?php 
			...
			/**
			 * echeanciers is a database object that can persist, delete and create forms for the echeanciers table
			 *
			 */
			
			class echeanciers extends baseModel
			{
			    ...
			    */
				public function __construct($rewriting=false)
				{
					...
					$this->tableName="echeanciers";
					
					parent::__construct($rewriting);
					
					
				$this->arrayFields["idecheanciers"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"PRI",
					...
					"formLabel"=>"idecheanciers"
				);
			
				$this->arrayFields["ech_is_payed"]=array
				(
					"type"=>"tinyint(4)",
					"canBeNull"=>"YES",
					"key"=>"",
					"default"=>"0",
					"extra"=>"",
					"formLabel"=>"ech_is_payed"
				);
			
				$this->arrayFields["ech_date_due"]=array
				(
					"type"=>"bigint(15)",
					"canBeNull"=>"YES",
					"key"=>"",
					"default"=>"",
					"extra"=>"",
					"formLabel"=>"ech_date_due"
				);
			
				$this->arrayFields["ech_date_payed"]=array
				(
					"type"=>"bigint(15)",
					"canBeNull"=>"YES",
					"key"=>"",
			    ...
			    */
				public function initFromDatas($arrayFilters)
				{
					...
					$sqlReq="SELECT * FROM echeanciers ".$theWhere." LIMIT 1";
			    ...
			    */
				public function delete()
				{
					if($this->isSavedElement)
					{
						...
						$sqlDelete="DELETE FROM echeanciers ".$theWhere."";
			    ...
			    */
				public function save($arrayFieldNames=null)
				{
					...
					$sqlInsert="INSERT INTO echeanciers(".implode(",",$toSave).") values(".implode(",",$toSaveBinding).")";
			    ...
			    */
				public function update($arrayFieldNames=null)
				{
					...
					if($this->isSavedElement)
					{
						...
						$sqlUpdate="UPDATE echeanciers SET ".implode(",",$toSave)." WHERE ".$this->getPrimaryKeyField()."=:".$this->getPrimaryKeyField()."";
			...
			/**
			 * echeanciersDatasManager is a database object that list datas from the echeanciers table
			 *
			 */
			
			class echeanciersDatasManager extends baseModel
			{
			    ...
			    */
				...
				public function refreshNumElements()
				{
					...
					$sqlCount="SELECT COUNT(*) AS numElements FROM echeanciers ".$this->getWhereClause();
				...
				{
					$sqlReq="SELECT * FROM echeanciers ".$this->getWhereClause()." ".$this->getLimitClause($page)."";
				...
				{
					$sqlReq="SELECT ".$columns." FROM echeanciers ".$supReq;
					$prepa=$this->dbh->prepare($sqlReq);
					if($qh)
					{$prepa->execute($qh);}
					else 
