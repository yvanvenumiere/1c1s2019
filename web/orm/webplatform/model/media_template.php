<?php 
			...
			/**
			 * media_template is a database object that can persist, delete and create forms for the media_template table
			 *
			 */
			
			class media_template extends baseModel
			{
			    ...
			    */
				public function __construct($rewriting=false)
				{
					...
					$this->tableName="media_template";
					
					parent::__construct($rewriting);
					
					
				$this->arrayFields["idmedia_template"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"PRI",
				...
				$this->arrayFields["medias_idmedias"]=array
				(
					"type"=>"int(11)",
					"canBeNull"=>"NO",
					"key"=>"MUL",
					"default"=>"",
					"extra"=>"",
					"formLabel"=>"medias_idmedias"
				);
			
					
				$this->arrayRelations["medias_idmedias"]=array
				(
					"assoc_table"=>"medias",
					"assoc_col"=>"idmedias"
				);
			    ...
			    */
				public function initFromDatas($arrayFilters)
				{
					...
					$sqlReq="SELECT * FROM media_template ".$theWhere." LIMIT 1";
			    ...
			    */
				public function delete()
				{
					if($this->isSavedElement)
					{
						...
						$sqlDelete="DELETE FROM media_template ".$theWhere."";
			    ...
			    */
				public function save($arrayFieldNames=null)
				{
					...
					$sqlInsert="INSERT INTO media_template(".implode(",",$toSave).") values(".implode(",",$toSaveBinding).")";
			    ...
			    */
				public function update($arrayFieldNames=null)
				{
					...
					if($this->isSavedElement)
					{
						...
						$sqlUpdate="UPDATE media_template SET ".implode(",",$toSave)." WHERE ".$this->getPrimaryKeyField()."=:".$this->getPrimaryKeyField()."";
			...
			/**
			 * media_templateDatasManager is a database object that list datas from the media_template table
			 *
			 */
			
			class media_templateDatasManager extends baseModel
			{
			    ...
			    */
				...
				public function refreshNumElements()
				{
					...
					$sqlCount="SELECT COUNT(*) AS numElements FROM media_template ".$this->getWhereClause();
				...
				{
					$sqlReq="SELECT * FROM media_template ".$this->getWhereClause()." ".$this->getLimitClause($page)."";
				...
				{
					$sqlReq="SELECT ".$columns." FROM media_template ".$supReq;
					$prepa=$this->dbh->prepare($sqlReq);
					if($qh)
					{$prepa->execute($qh);}
					else 
