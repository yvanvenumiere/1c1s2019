<?php
function getContenu($typeGeneration,$table)
{
	$retour;
	switch($typeGeneration)
	{
		case 'simple':
			...
			/**
			 * '.$table.' is a database object that can persist, delete and create forms for the '.$table.' table
			 *
			 */
			...
			class '.$table.' extends baseModel
			{
			    ...
			    */
				public function initFromDatas($arrayFilters)
				{
					...
					$sqlReq="SELECT * FROM '.$table.' ".$theWhere." LIMIT 1";
			    ...
			    */
				public function delete()
				{
					if($this->isSavedElement)
					{
						...
						$sqlDelete="DELETE FROM '.$table.' ".$theWhere."";
			    ...
			    */
				public function save($arrayFieldNames=null)
				{
					...
					$sqlInsert="INSERT INTO '.$table.'(".implode(",",$toSave).") values(".implode(",",$toSaveBinding).")";
			    ...
			    */
				public function update($arrayFieldNames=null)
				{
					...
					if($this->isSavedElement)
					{
						...
						$sqlUpdate="UPDATE '.$table.' SET ".implode(",",$toSave)." WHERE ".$this->getPrimaryKeyField()."=:".$this->getPrimaryKeyField()."";
			...
			/**
			 * '.$table.'DatasManager is a database object that list datas from the '.$table.' table
			 *
			 */
			
			class '.$table.'DatasManager extends baseModel
			{
			    ...
			    */
				...
				public function refreshNumElements()
				{
					...
					$sqlCount="SELECT COUNT(*) AS numElements FROM '.$table.' ".$this->getWhereClause();
				...
				{
					$sqlReq="SELECT * FROM '.$table.' ".$this->getWhereClause()." ".$this->getLimitClause($page)."";
				...
				{
					$sqlReq="SELECT ".$columns." FROM '.$table.' ".$supReq;
					$prepa=$this->dbh->prepare($sqlReq);
					if($qh)
					{$prepa->execute($qh);}
					else 
